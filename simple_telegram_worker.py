...
class TelegramWorker:
    ...
    def extract_size_from_item(self, item: Dict[str, Any]) -> str:
        """Extract size information from item data with improved filtering"""
        size = ""
        ...
        if not size:
            ...
            for text in texts_to_check:
                ...
                size = self._extract_size_with_validation(text)
                if size:
                    break
        
        return size.strip() if size else ""
    
    def _extract_size_with_validation(self, text: str) -> str:
        """Extract size with validation to avoid false positives"""
        if not text:
            return ""
        ...
        size_patterns = [
            ...
            r'(?:куртка|рубашка|платье|джинсы|брюки|футболка|свитер|костюм|пальто|блузка|юбка|шорты|толстовка|худи|кофта)\s+.*?(\d+[-–]\d+\s*\([XSMLXL]+\))',
            r'(?:куртка|рубашка|платье|джинсы|брюки|футболка|свитер|костюм|пальто|блузка|юбка|шорты|толстовка|худи|кофта)\s+.*?(\d+\s*\([XSMLXL]+\))',
            r'(?:куртка|рубашка|платье|джинсы|брюки|футболка|свитер|костюм|пальто|блузка|юбка|шорты|толстовка|худи|кофта)\s+.*?\b([XSMLXL]{1,3})\b',
        ]
        ...
        for pattern in size_patterns:
            ...
            if match:
                ...
                if self._is_valid_clothing_size(potential_size, text):
                    return potential_size
        
        return ""
    
    def _is_valid_clothing_size(self, size: str, context: str) -> bool:
        """Validate if the extracted text is actually a clothing size"""
        if not size:
            return False
        ...
        for pattern in valid_patterns:
            if re.match(pattern, size, re.IGNORECASE):
                # For numeric sizes, require clothing context
                if re.match(r'^\d', size):
                    if has_clothing_context:
                        ...
                        if numeric_part:
                            ...
                            # Reasonable clothing size range (european sizes mostly 36-62)
                            if 30 <= num <= 70:
                                return True
                    return False
                else:
